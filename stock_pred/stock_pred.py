...
# Import data         data = pd.read_csv('data_stocks.csv'); data = data.drop(['DATE'], 1); data.head()

data = pd.read_csv('EURCHF.csv',header=None, usecols = [2,]);
data.columns = ['open',]
data = data[['open']]

lookback=60; lookforward=10

df = pd.concat([data.shift(-lookforward),]+[data.shift(i) for i in range(lookback)],axis=1).dropna()
df.columns = ['open_target',]+['open_'+str(i) for i in range(lookback)]
...
out      = tf.transpose(tf.layers.dense(inputs=hidden_3, units=levels[4], activation=actn_fn, kernel_initializer = weight_initializer))
